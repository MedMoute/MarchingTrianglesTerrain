    ...
    protected override Func<Triangulation, Triangulation> DelegateAction
    {
        get
        {
            return t =>
            {
                ...
                {
                    t.Debug();
                    throw new Exception(String.Format("Unexpected state : the edge ({0}, {1}) is supposed to already exist but was not found.",p0Idx,p1Idx));
                }
                ...
                //var A = HexTerrainCell.GetSignedArea([t.Vertices[p0Idx], t.Vertices[newIdx], t.Vertices[p1Idx]]);
                //t.TrianglesByEdges.Add(t.TrianglesByEdges.Count, [newEdge0idx, newEdge1idx, edge]);
